...
const LeaderRanking = (props: TData) => {
  ...
  const rankOneItem = data?.find(item => parseInt(getRank(item, type)) === 1);
  const otherItems = data
    ?.filter(item => parseInt(getRank(item, type)) !== 1)
    ?.sort((a, b) => {
      const rankA = parseInt(getRank(a, type));
      const rankB = parseInt(getRank(b, type));
      const modifiedSortOrder = sortOrder.filter(rank => rank !== 1); // remove rank 1 from sort order
      const indexA = modifiedSortOrder.indexOf(rankA);
      const indexB = modifiedSortOrder.indexOf(rankB);

      // Sort based on modified sortOrder, fallback to maintaining the original order if not found
      if (indexA !== -1 && indexB !== -1) {
        return indexA - indexB;
      }
      return 0; // Maintain original order if rank not in modified sortOrder
    });
  const sortedData = [
    ...otherItems.slice(0, otherItems.length / 2),
    rankOneItem,
    ...otherItems.slice(otherItems.length / 2),
  ];
